...
def train_test(data: pd.DataFrame, label: str):
...
def train_classifier(X_train, Y_train, X_test, Y_test, algo_name, max_iter=300):
    #     clf = SGDClassifier(loss="hinge", penalty="l2", max_iter=max_iter)
    #     clf.fit(X_train, Y_train)
    #     Y_predict = clf.predict(X_test)
    if algo_name=='Support Vector Machine':
        model = OneVsOneClassifier(LinearSVC(random_state=0, max_iter=max_iter))
    ...
    train_df = pd.DataFrame({"accuracy": [metrics.accuracy_score(targs, preds)],
                             "precision": [metrics.precision_score(targs, preds, average='macro')],
                             "recall": [metrics.recall_score(targs, preds, average='macro')],
...
def undersampling(X_train, y_train):
    ...
    df_count = count3.groupby(['quality']).size().reset_index(name='counts')
    min_count = np.min(df_count['counts'])

    if min_count<3:
        nr = NearMiss(n_neighbors=int(min_count))
    else:
        nr = NearMiss()
